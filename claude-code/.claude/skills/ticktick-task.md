...
> Tool names below are the server's real names (see `references/ticktick-mcp-tools-reference.md`). On Claude Code they appear as `mcp__dida365__<tool>`. The server does NOT provide `get_engaged_tasks` / `search_tasks` / `mcp_ticktick_*` — those are fictional. Task creation uses a `task` object with `title` + `projectId` (+ optional `content`, `dueDate`, `startDate`, `priority`, `tags`).
